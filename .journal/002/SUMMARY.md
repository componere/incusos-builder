...
- Boot gate (1.E): OPEN — macOS cannot diagnose (Secure Boot hard gate; shipped UKIs configure no console). Interim = release checklist; Phase 5.2 gets one time-boxed Linux run (attempt-C command line recorded in `.journal/002/spike-1e-boot.md`).
...
- Phase 1 (#8): five spike findings docs (now in this session folder) + quarantined spike modules under `spikes/`; adversarial review corrected 12 findings incl. two P0s (false swtpm premise; my own rooted-path ReadDir misuse).
...
- Spike findings: `.journal/002/spike-1{a,b,c,d,e}-*.md` (moved here from `docs/notes/` on master).
- Boot-gate command lines: `.journal/002/spike-1e-boot.md` (attempt C).
