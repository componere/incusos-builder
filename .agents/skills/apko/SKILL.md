...
name: apko
description: >
  Assemble the runtime OCI image for incusos-builder with apko (`apko.yaml`) — the
...
- `contents.packages`: `wolfi-baselayout`, `ca-certificates-bundle`, `tzdata`, and
  `incusos-builder@local`. The `@local` suffix pins the package to the `@local` repository,
...
- `entrypoint.command: /usr/bin/incusos-builder` — where the `go/build` melange pipeline
  installs the binary.
...
`incusos-builder:dev` for local running/inspection. The apko step is:

```bash
apko build apko.yaml incusos-builder:dev image.tar \
  --arch "$arch" \
...
- **The retag is required, not cosmetic.** A single-arch `apko build` loads into Docker under
  an arch-suffixed tag (`incusos-builder:dev-amd64` / `-arm64`, using the Go arch name from
  `go env GOARCH`). The task retags to the plain `incusos-builder:dev` so it is easy to
