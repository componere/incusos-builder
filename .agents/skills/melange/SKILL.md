...
- `package`: `name: incusos-builder`, `version: "0.1.1"` (release-please marker), `epoch: 0`.
...
- `pipeline: - uses: go/build` with `packages: ./cmd/incusos-builder`,
  `output: incusos-builder`, `go-package: go-1.26`, `modroot: .`, `strip: "-s -w"`, the
  ...
  and installs to `/usr/bin/<output>` — apko's entrypoint `/usr/bin/incusos-builder` depends on
...
- Do not override `output:` in the pipeline without updating `apko.yaml`'s `entrypoint.command`
  and `contents.packages` — the binary path `/usr/bin/incusos-builder` is contractual.
